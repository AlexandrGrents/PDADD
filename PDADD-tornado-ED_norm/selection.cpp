...
void findCandidats_norm(float *t, double**u_and_q,	float r, int N, int n, int L, int p, int **I, int *Insert, int *Bottom,	int *Count)
{
	int NIL = -1;
#pragma omp parallel num_threads(p)
	{
...
void get_C_norm(float *t,  float **C, int *Candidats, int *H, int n)
{
	for (int i=0; i<*H; i++)
	{
		for (int j = 0; j < n; j++)
		{
			C[i][j] = t[Candidats[i] + j];
		}
	}
}
