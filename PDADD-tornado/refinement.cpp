...
void findDiscords_norm(float *t, float ** u_and_q, float * u_and_q_c, float *C, int *Candidats, float r, int N, int n, 
		...
		for (int i = 0; i < N; i++)
		{
			for (int j = 0; j < H; j++)
			{
				...
				{	
					...
					if (B[iam][j])
					{
						B[iam][j] = (ED_norm(t+i, C + j * n, n, u_and_q[0][i], u_and_q_c[j], 
							u_and_q[1][i], u_and_q_c[j+1]) > r);
					}
					else continue;
				}
...
void refinement_norm(float *t, float **u_and_q, float * u_and_q_c, bool **B, float *C, int *Candidats, int *Discords, 
